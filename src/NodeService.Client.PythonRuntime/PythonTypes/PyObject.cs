...
namespace Python.Runtime
{
    ...
    public partial class PyObject : DynamicObject, IDisposable, ISerializable
    {
        ...
        {
            GC.SuppressFinalize(this);
            return StolenReference.Take(ref rawPtr);
        }
        ...
        internal PyType PyType => GetPythonType();
        ...
        ManagedType? InternalManagedObject => ManagedType.GetManagedObject(Reference);
        ...
        string? DebuggerDisplay
        {
            get
            {
                if (DebugUtil.HaveInterpreterLock())
                    return ToString();
                var obj = InternalManagedObject;
                return obj is { }
                    ? obj.ToString()
                    : $"pyobj at 0x{rawPtr:X} (get Py.GIL to see more info)";
            }
        }
        ...
        {
            using var _ = Py.GIL();
            result = CheckNone(GetAttr(binder.Name));
            return true;
        }
        ...
        public override bool TryConvert(ConvertBinder binder, out object? result)
        {
            ...
            if (Converter.ToManaged(obj, binder.Type, out result, false))
            {
                return true;
            }
            ...
            if (binder.Type == typeof(System.Collections.IEnumerable) && IsIterable())
            {
                result = new PyIterable(Reference);
                return true;
            }
        ...
        {
            int result = Runtime.PyObject_RichCompareBool(obj, arg.obj, op);
            @out = result != 0;
            if (result < 0)
            {
                Exceptions.Clear();
        ...
        public override bool TryBinaryOperation(BinaryOperationBinder binder, object arg, out object? result)
        {
            ...
            switch (binder.Operation)
            {
                ...
                case ExpressionType.Divide:
                    res = Runtime.PyNumber_TrueDivide(obj, ((PyObject)arg).obj);
                    break;
                case ExpressionType.DivideAssign:
                    res = Runtime.PyNumber_InPlaceTrueDivide(obj, ((PyObject)arg).obj);
                    break;
                ...
                case ExpressionType.GreaterThan:
                    return TryCompare((PyObject)arg, Runtime.Py_GT, out result);
                case ExpressionType.GreaterThanOrEqual:
                    return TryCompare((PyObject)arg, Runtime.Py_GE, out result);
                ...
                case ExpressionType.LessThan:
                    return TryCompare((PyObject)arg, Runtime.Py_LT, out result);
                case ExpressionType.LessThanOrEqual:
                    return TryCompare((PyObject)arg, Runtime.Py_LE, out result);
                ...
                case ExpressionType.NotEqual:
                    return TryCompare((PyObject)arg, Runtime.Py_NE, out result);
                case ExpressionType.Equal:
                    return TryCompare((PyObject)arg, Runtime.Py_EQ, out result);
        ...
        public override bool TryUnaryOperation(UnaryOperationBinder binder, out object? result)
        {
            ...
            switch (binder.Operation)
            {
                case ExpressionType.Negate:
                    res = Runtime.PyNumber_Negative(obj);
                    break;
                case ExpressionType.UnaryPlus:
                    res = Runtime.PyNumber_Positive(obj);
                    break;
                case ExpressionType.OnesComplement:
                    res = Runtime.PyNumber_Invert(obj);
                    break;
                case ExpressionType.Not:
                    r = Runtime.PyObject_Not(obj);
                    result = r == 1;
                    if (r == -1) Exceptions.Clear();
                    return r != -1;
                case ExpressionType.IsFalse:
                    r = Runtime.PyObject_IsTrue(obj);
                    result = r == 0;
                    if (r == -1) Exceptions.Clear();
                    return r != -1;
