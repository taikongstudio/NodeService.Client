namespace Python.Runtime.Codecs
{
    ...
    public sealed class DecoderGroup: IPyObjectDecoder, IEnumerable<IPyObjectDecoder>, IDisposable
    {
        ...
        /// </summary>
        public void Clear() => decoders.Clear();

        /// <inheritdoc />
        public bool CanDecode(PyType objectType, Type targetType)
            => decoders.Any(decoder => decoder.CanDecode(objectType, targetType));
        /// <inheritdoc />
        public bool TryDecode<T>(PyObject pyObj, out T? value)
        {
            if (pyObj is null) throw new ArgumentNullException(nameof(pyObj));

            var decoder = GetDecoder(pyObj.GetPythonType(), typeof(T));
            if (decoder is null)
            {
                value = default;
                return false;
        ...
        /// <inheritdoc />
        public IEnumerator<IPyObjectDecoder> GetEnumerator() => decoders.GetEnumerator();
        IEnumerator IEnumerable.GetEnumerator() => decoders.GetEnumerator();

        public void Dispose()
        {
            foreach (var decoder in decoders.OfType<IDisposable>())
            {
                decoder.Dispose();
            }
            decoders.Clear();
