...
namespace Python.Runtime
{
    ...
    internal class InterfaceObject : ClassBase
    {
        ...
        public NewReference TryWrapObject(object impl)
            => type.Valid
                ? CLRObject.GetReference(impl, ClassManager.GetClass(type.Value))
                : Exceptions.RaiseTypeError(type.DeletedMessage);
        ...
        protected override void OnDeserialization(object sender)
        {
            ...
            if (type.Valid)
            {
                ctor = TryGetCOMConstructor(type.Value);
            }
        }
    }
}
