...
namespace Python.Runtime.Mixins
{
    class CollectionMixinsProvider : IPythonBaseTypeProvider, IDisposable
    {
        ...
        {
            mixinsModule = mixinsModule ?? throw new ArgumentNullException(nameof(mixinsModule));
        }
        ...
        public IEnumerable<PyType> GetBaseTypes(Type type, IList<PyType> existingBases)
        {
            ...
            {
                newBases.Add(new PyType(Mixins.GetAttr("MutableMappingMixin")));
            }
            else if (interfaces.Contains(typeof(IReadOnlyDictionary<,>)))
            {
                newBases.Add(new PyType(Mixins.GetAttr("MappingMixin")));
            }
        ...
        public void Dispose()
        {
            if (mixinsModule.IsValueCreated)
            {
                mixinsModule.Value.Dispose();
            }
        }
    }
}
