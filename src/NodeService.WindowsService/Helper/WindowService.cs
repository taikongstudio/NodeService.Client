...
namespace NodeService.WindowsService.Helper
{
    ...
    public static class ServiceHelper
    {
        ...
        {
            nint scm = OpenSCManager();

            nint service = nint.Zero;
            try
            {
                service = Win32Class.CreateService(scm, serviceName, displayName, Win32Class.SERVICE_ALL_ACCESS, Win32Class.SERVICE_WIN32_OWN_PROCESS, startType, Win32Class.SERVICE_ERROR_NORMAL, binaryFilePath, null, nint.Zero, ProcessDependencies(dependencies), GetServiceAccountName(account), null);
        ...
        /// 打开服务管理器
        /// </summary>
        private static nint OpenSCManager()
        {
            nint scm = Win32Class.OpenSCManager(null, null, Win32Class.SC_MANAGER_ALL_ACCESS);
        ...
        private static nint OpenService(string serviceName, out nint scmHandle)
        {
            scmHandle = OpenSCManager();

            nint service = Win32Class.OpenService(scmHandle, serviceName, Win32Class.SERVICE_ALL_ACCESS);
        ...
        public static bool TryGetServiceState(string serviceName, out ServiceState serviceState)
        {
            ...
            nint scHandle = OpenSCManager();
            if (scHandle == nint.Zero)
            {
                return false;
            }
        ...
        /// 停止服务
        /// </summary>
        private static void StopService(nint service)
        {
        ...
        private static string[] EnumDependentServices(nint serviceHandle, EnumServiceState state)
        {
            int bytesNeeded = 0; //存放从属服务的空间大小，由API返回
            int numEnumerated = 0; //从属服务数，由API返回

            //先尝试以空结构获取，如获取成功说明从属服务为空，否则拿到上述俩值
            if (Win32Class.EnumDependentServices(serviceHandle, state, nint.Zero, 0, ref bytesNeeded, ref numEnumerated))
            {
                return new string[0];
            }
            ...
            //在非托管区域创建指针
            nint structsStart = Marshal.AllocHGlobal(new nint(bytesNeeded));
            try
            {
                ...
                for (int i = 0; i < numEnumerated; i++)
                {
                    ...
                    nint ptr = new nint(structsStartAsInt64 + i * sizeOfStruct); //根据起始指针、结构次序和结构大小推算各结构起始指针
        ...
        /// 获取服务状态
        /// </summary>
        private static ServiceState GetServiceStatus(nint service)
        {
            Win32Class.SERVICE_STATUS status = new Win32Class.SERVICE_STATUS();
        ...
        /// 等候服务至目标状态
        /// </summary>
        private static bool WaitForStatus(nint serviceHandle, ServiceState desiredStatus, TimeSpan timeout)
        {
        ...
        private static class Win32Class
        {
            ...
            public static extern bool ChangeServiceConfig2(nint serviceHandle, uint infoLevel, ref SERVICE_DESCRIPTION serviceDesc);

            [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            public static extern nint OpenSCManager(string machineName, string databaseName, int dwDesiredAccess);

            [DllImport("advapi32.dll", SetLastError = true, CharSet = CharSet.Auto)]
            public static extern nint OpenService(nint hSCManager, string lpServiceName, int dwDesiredAccess);

            [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            public static extern nint CreateService(nint hSCManager, string lpServiceName, string lpDisplayName, int dwDesiredAccess, int dwServiceType, ServiceStartType dwStartType, int dwErrorControl, string lpBinaryPathName, string lpLoadOrderGroup, nint lpdwTagId, string lpDependencies, string lpServiceStartName, string lpPassword);
            ...
            [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            public static extern bool QueryServiceStatus(nint hService, ref SERVICE_STATUS lpServiceStatus);

            [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            public static extern bool DeleteService(nint serviceHandle);

            [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            public static extern bool ControlService(nint hService, int dwControl, ref SERVICE_STATUS lpServiceStatus);

            [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            public static extern int StartService(nint hService, int dwNumServiceArgs, nint npServiceArgVectors);

            [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            public static extern bool EnumDependentServices(nint serviceHandle, EnumServiceState serviceState, nint bufferOfENUM_SERVICE_STATUS, int bufSize, ref int bytesNeeded, ref int numEnumerated);
