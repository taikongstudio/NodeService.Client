...
{
    public class UpdateDeviceConfigurationService : BackgroundService
    {
        private string _localServerConfigPath;
        private string _configPath;
        private string _remoteServerConfigRoot;
        private int _hashCode;
        private ILogger<UpdateDeviceConfigurationService> _logger;
        private List<Process> _pluginProcesses;
        private bool _isUpdating;
        DeviceConfiguration currentFtpServerConfig;
        private string? _pluginPath;

        public UpdateDeviceConfigurationService(ILogger<UpdateDeviceConfigurationService> logger)
        {
            _logger = logger;
        }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                _logger.LogInformation($"Start {nameof(UpdateDeviceConfigurationService)}");
        ...
        private void UpdateImpl(CancellationToken cancellationToken)
        {
            try
            {
                var loadConfigResult = DeviceConfiguration.TryLoadServerConfig(
                ...
                var server = localFtpServerConfig.host;
                ...
                memoryStream.Position = 0;
                var remoteFtpServerConfig = JsonSerializer.Deserialize<DeviceConfiguration>(memoryStream);
                ...
                if (ShouldTestRemoteCondig(currentFtpServerConfig, remoteFtpServerConfig) && !TestRemoteConfig(remoteFtpServerConfig))
                {
                    MyLog($"[UpdateImpl]Could not connect remote config " +
                        $"server:{remoteFtpServerConfig.host}" +
        ...
        private bool ValidateConfig(DeviceConfiguration ftpServerConfig)
        {
            ...
            if (ftpServerConfig.plugins != null)
            {
                ...
                {
                    var localPluginDir = Path.Combine(_pluginPath, pluginInfo.plugin_name, pluginInfo.version);
                    if (!Directory.Exists(localPluginDir))
                    {
                        _logger.LogError($"missing directory:{localPluginDir}");
                        return false;
                    }
                    var entryPoint = Path.Combine(localPluginDir, pluginInfo.entry_point);
        ...
        private bool ShouldTestRemoteCondig(DeviceConfiguration currentFtpServerConfig, DeviceConfiguration remoteFtpServerConfig)
        {
            return currentFtpServerConfig.host != remoteFtpServerConfig.host
                ||
        ...
        private int UpdatePluginFiles(FtpClient ftpClient, DeviceConfiguration remoteFtpServerConfig, int updateCount, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    KillPluginProcessByProcessName(pluginInfo.plugin_name, cancellationToken);
                    for (int i = 0; i < 10; i++)
                    {
                        if (cancellationToken.IsCancellationRequested)
                        {
                        ...
                        if (updateStatus == UpdatePluginResult.Failed)
                        {
                            ...
                            _logger.LogError($"Download fail:{pluginInfo.plugin_name}");
                            continue;
                        }
                        if (updateStatus == UpdatePluginResult.Success)
                        {
                            _logger.LogInformation($"Update plugin{pluginInfo.plugin_name} success");
                            updateCount++;
                        }
                        else if (updateStatus == UpdatePluginResult.Skipped)
                        {
                            _logger.LogInformation($"Skip Update plugin{pluginInfo.plugin_name}");
                        }
        ...
        private bool TestRemoteConfig(DeviceConfiguration remoteFtpServerConfig)
        {
            try
            {
                var server = remoteFtpServerConfig.host;
        ...
        private void StartPluginProcesses(DeviceConfiguration ftpServerConfig, bool skipIfExits)
        {
            try
            {
                if (ftpServerConfig.plugins == null)
                ...
                foreach (var pluginInfo in ftpServerConfig.plugins)
                {
                    try
                    {
                        ...
                        {
                            _logger.LogInformation($"Skip launch {pluginInfo.plugin_name}");
                            continue;
                        }
                        var pluginWorkerDir = Path.Combine(_pluginPath, pluginInfo.plugin_name, pluginInfo.version);
                        var exePath = Path.Combine(_pluginPath, pluginInfo.plugin_name, pluginInfo.version, pluginInfo.entry_point);
                        if (!File.Exists(exePath))
                        {
                            _logger.LogError($"[Plugin]Could not found plugin {pluginInfo.plugin_name} exe:{exePath}");
                            continue;
                        }

                        if (skipIfExits)
                        {
                            var processes = Process.GetProcessesByName(pluginInfo.plugin_name);

                            if (processes.Any())
                            {
                                _logger.LogError($"[Plugin]skip create {pluginInfo.plugin_name} exe:{exePath}");
                                continue;
                            }
                        }
                        ...
                        {
                            _logger.LogError($"[Plugin]Could not launch plugin {pluginInfo.plugin_name} exe:{exePath} args:{cmdArgs}");
            ...
            string tempPluginDirectory)
        {
            try
            {
                var localPluginDir = Path.Combine(localPluginDirectory, pluginInfo.plugin_name, pluginInfo.version);
                ...
                else
                {
                    ...
                    if (File.Exists(hashFile))
                    {
                        ...
                        {
                            _logger.LogInformation($"Skip update plugin:{pluginInfo.plugin_name} because same hash");
                            return UpdatePluginResult.Skipped;
                        }
                        else
                        {
                            _logger.LogInformation($"updating plugin:{pluginInfo.plugin_name}");
                        }
                    }
                    CleanupFiles(localPluginDir);
                }
                var downloadUrl = Path.Combine(remotePluginRootDir, pluginInfo.plugin_name, pluginInfo.version, RuntimeInformation.OSArchitecture.ToString(), pluginInfo.downloadUrl);
        ...
        private int UpdateConfigFiles(FtpClient ftpClient, DeviceConfiguration? remoteFtpServerConfig, int updateCount)
        {
        ...
        private bool UpdateConfigFilesImpl(FtpClient ftpClient,
            DeviceConfiguration remoteFtpServerConfig,
