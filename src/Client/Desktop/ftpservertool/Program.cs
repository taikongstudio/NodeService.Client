...
namespace ftpservertool
{
    internal class Program
    {
        ...
        private static void MergeExt1(MySqlConfig mySqlConfig, string channel)
        {
            ...
            var machineInfoList = SqlHelper.GetMachineInfoList(mySqlConfig);
            ...
            var dataList = machineInfoList.ToList();
            ...
            string channel)
        {
            try
            {
                ...
                {
                    var machineInfo = machineInfoList.FirstOrDefault(
                        x => string.Equals(x.computer_name,
                        item.computer_name,
                        StringComparison.OrdinalIgnoreCase));
                    if (machineInfo != null)
                    {
                        machineInfo.install_status = item.install_status;
                    }
                    else
                    {
                        machineInfo = new MachineInfo();
                        machineInfo.computer_name = item.computer_name;
                        machineInfo.factory_name = factoryNameDict[channel];
                        machineInfo.install_status = true;
                        machineInfo.test_info = string.Empty;
                        machineInfo.lab_area = string.Empty;
                        machineInfo.lab_name = string.Empty;
                        machineInfo.login_name = string.Empty;
                        machineInfo.host_name = string.Empty;
                        machineInfo.update_time = string.Empty;
                        machineInfo.version = string.Empty;
                    }
                    mergedList.Add(machineInfo);
                }
        ...
        private static void GenMarccorStatus(Options options, MySqlConfig mySqlConfig, string? csvFilePathString, string? csvOutFilePathString)
        {
            try
            {
                ...
                foreach (var item in result)
                {
                    var oldItem = machineInfoList.FirstOrDefault(x => x.computer_name == item.computer_name);
                    if (oldItem != null)
                    {
        ...
        private static void UpdateMachineInfo(Options options, MySqlConfig mySqlConfig, string csvFilePathString)
        {
            try
            {
                ...
                SqlHelper.InsertOrUpdate(mySqlConfig, machineInfoList, Console.WriteLine).Wait();
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex);
            ...
            try
            {
                ...
                using (var ftpClient = new FtpClient(options.host, options.username, options.password, options.port))
                {
                    ...
                    foreach (var machineDir in ftpClient.GetListing("JobsWorkerDaemonService").OrderBy(x => x.GetHashCode()))
                    {
                        ...
                        //var machineInfo = machineList.FirstOrDefault(
                        ...
                        //    factoryName != machineInfo.factory_name
                        //    )
                        //{
                        //    continue;
                        //}
                        ...
                        //else
                        //{
                        //    //Console.WriteLine($"{machineInfo.factory_name} {machineInfo.computer_name}");
                        //}
        ...
        public static void CheckConfigDir(Options options, MySqlConfig mySqlConfig,string channel)
        {
            using (var ftpClient = new FtpClient(options.host, options.username, options.password, options.port))
            {
                ...
                foreach (var machineDir in ftpClient.GetListing("JobsWorkerDaemonService").OrderBy(x => x.GetHashCode()))
                {
                    ...
                    else
                    {
                        ...
                        var machineInfo = machineList.FirstOrDefault(x =>
                        ...
                        machineInfo = new MachineInfo();
                        machineInfo.computer_name = machineDir.Name;
                        machineInfo.factory_name = factoryNameDict[channel];
                        machineInfo.install_status = true;
                        machineInfo.test_info = string.Empty;
                        machineInfo.lab_area = string.Empty;
            ...
            static void CheckLog(FtpClient ftpClient, string machineName, IEnumerable<FtpListItem> logFiles, DateTime dateTime, MachineInfo machineInfo)
            {
                ...
                foreach (var checkItem in checkLists)
                {
                    ...
                    {
                        Console.WriteLine($"{machineName,-20} {machineInfo.factory_name,-15} {machineInfo.lab_name,-15} {machineInfo.lab_area,-15} :missing {checkItem}");
                    }
                }
            }
        }
        ...
        public static void CheckLogDate(Options options, MySqlConfig mySqlConfig)
        {
            using (var ftpClient = new FtpClient(options.host, options.username, options.password, options.port))
            {
                ...
                foreach (var listItem in ftpClient.GetListing("JobsWorkerDaemonService").OrderBy(x => x.GetHashCode()))
                {
                    ...
                    MachineInfo machineInfo = FirstOrDefault(machineList, listItem.Name);
                    ...
                    {
                        CheckLog(ftpClient, listItem.Name, logFiles, endDate, machineInfo);
                    }
        ...
        public static void CheckUpdate(Options options, MySqlConfig mySqlConfig, DateTime datetime, string keywords)
        {
            ...
            using (var ftpClient = new FtpClient(options.host, options.username, options.password, options.port))
            {
                ...
                {
                    MachineInfo machineInfo = FirstOrDefault(machineList, item.Key);
                    if (item.Value > datetime)
                    {
                        Console.WriteLine($"{item.Key,-30}  {machineInfo.factory_name,-15} {machineInfo.lab_name,-15} {machineInfo.lab_area,-15}  {item.Value}");
                    }
                    else
                    {
                ...
                {
                    MachineInfo machineInfo = FirstOrDefault(machineList, machineName);
                    Console.WriteLine($"{machineName} {machineInfo.factory_name,-15} {machineInfo.lab_name,-15} {machineInfo.lab_area,-15} ");
                }
            }
