    ...
    internal class Program
    {
        ...
        private static void AnalysisAllMachineUsages(Options options, MySqlConfig mySqlConfig)
        {
            ...
            foreach (var machineDir in mainClient.GetListing("/JobsWorkerDaemonService/"))
            {
                ...
                var machineInfo = machineList.FirstOrDefault(x => string.Equals(machineDir.Name, x.computer_name, StringComparison.OrdinalIgnoreCase));
                if (machineInfo == null)
                {
                ...
                var cache = cacheList.FirstOrDefault(x => x.MachineName == machineDir.FullName);
                if (cache != null)
                {
                    machineInfo.usages = cache.Usages;
                    continue;
                }

                machineInfo.has_ftp_dir = true;
                SqlHelper.InsertOrUpdate(mySqlConfig, new MachineInfo[] { machineInfo }, Console.WriteLine).Wait();
                ...
                taskParameters.MachineInfo = machineInfo;
                taskParameters.MachineName = machineDir.Name;
            ...
            static void RunTask(TaskParameters taskParameters)
            {
                try
                {
                    Console.WriteLine($"Processing {taskParameters.MachineName}");
                    ...
                    {
                        Console.WriteLine($"{taskParameters.MachineName} no usages");
                        return;
                    }
                    Console.Write($"{taskParameters.MachineName}  set {taskParameters.MachineInfo.usages} to ");
                    ...
                    lock (taskParameters.Lock)
                    {
                        ...
                        var cache = cacheList.FirstOrDefault(x => x.MachineName == taskParameters.MachineName);
                        if (cache == null)
                        {
                            cache = new Cache()
                            {
                                MachineName = taskParameters.MachineName,
