...
namespace JobsWorkerNode.Helpers
{
    internal class SqlHelper
    {
        public static async Task<bool> InsertOrUpdate(MySqlConfig mySqlConfig, IEnumerable<MachineInfo> machineInfoList, ILogger logger)
        {
            try
            {
                ...
                foreach (var machineInfo in machineInfoList)
                {
                    if (await exists(connection, machineInfo))
                    {
                        ...
                        logger.LogInformation($"updated {machineInfo.computer_name}");
                    }
                    else
                    {
                        await Insert(connection, machineInfo);
                        logger.LogInformation($"inserted {machineInfo.computer_name}");
                    }
                }
    ...
    "values (@factory_name,@host_name,@test_info,@lab_area,@lab_name,@computer_name,@login_name,@install_status,@update_time,@version,@ip_addresses);";
        ...
        {
            var stm = "update machine_info set install_status=@install_status,update_time=@update_time,version=@version,ip_addresses=@ip_addresses where computer_name =@computer_name;";
            using var cmd = new MySqlCommand(stm, mySqlConnection);
            cmd.Parameters.Add(new MySqlParameter(nameof(machineinfo.computer_name), machineinfo.computer_name));
            cmd.Parameters.Add(new MySqlParameter(nameof(machineinfo.install_status), machineinfo.install_status));
            cmd.Parameters.Add(new MySqlParameter(nameof(machineinfo.update_time), machineinfo.update_time));
        ...
        public static List<MachineInfo> GetMachineInfoList(MySqlConfig mySqlConfig, ILogger logger)
        {
