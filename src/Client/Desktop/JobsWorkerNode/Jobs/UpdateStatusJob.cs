    ...
    public class UpdateStatusJob : JobBase
    {
        ...
        public override async Task Execute(IJobExecutionContext context)
        {
            try
            {
                ...
                if (!TryDownloadMachineConfigFromFtpServer(ftpClient, remotePath, out MachineInfo? current_machine_info)
                    ||
        ...
        private MachineInfo EnsureMachineInfo(MySqlConfig mySqlConfig, string factory_name, string hostName)
        {
            MachineInfo? current_machine_info;
            ...
            if (current_machine_info == null)
            {
                ...
                current_machine_info = new MachineInfo();
        ...
        private bool TryDownloadMachineConfigFromFtpServer(FtpClient ftpClient, string remotePath, out MachineInfo? machineInfo)
        {
        ...
        private void UploadMachineInfoToFtp(FtpClient ftpClient, string remotePath, MachineInfo? current_machine_info)
        {
        ...
        private async Task UploadAllMachineInfoToFtpAsync(FtpClient ftpClient, MySqlConfig mySqlConfig)
        {
            ...
            await SqlHelper.InsertOrUpdate(mySqlConfig, machineInfoList, Logger);
        }
        ...
        private void DownloadMachineListFromFtp(FtpClient ftpClient, List<MachineInfo> machineInfoList)
        {
            foreach (var machineDir in ftpClient.GetListing("JobsWorkerDaemonService"))
            {
                ...
                if (DownloadMachineInfo(ftpClient, remotePath, out MachineInfo machineInfo))
                {
        ...
        private bool DownloadMachineInfo(FtpClient ftpClient, string remotePath, out MachineInfo machineInfo)
        {
            machineInfo = null;
            using (var memoryStream = new MemoryStream())
            {
                ...
                {
                    memoryStream.Position = 0;
                    machineInfo = JsonSerializer.Deserialize<MachineInfo>(memoryStream);
